...
scale_colour_lcbc <- function(palette = "main",
                              discrete = TRUE,
                              reverse = FALSE,
                              ...) {
  ...
  if (discrete) {
    discrete_scale("color", paste0("lcbc_", palette), palette = pal, ...)
  } else {
...
scale_fill_lcbc <- function(palette = "main", discrete = TRUE, reverse = FALSE, ...) {
  ...
  if (discrete) {
    discrete_scale("fill", paste0("lcbc_", palette), palette = pal, ...)
  } else {
...
scale_colour_proj <- function(palette = "main", discrete = TRUE, reverse = FALSE, ...) {
  ...
  if(palette == "named"){
    ...
    discrete_scale("color", paste0("proj_", palette), palette = pal, ...)
  } else {
    scale_colour_gradientn(colours = pal(256), ...)
  }
}
...
scale_fill_proj <- function(palette = "main", discrete = TRUE, reverse = FALSE, ...) {
  ...
  if(palette == "named"){
    ...
    discrete_scale("fill", paste0("proj_", palette), palette = pal, ...)
  } else {
    scale_fill_gradientn(colours = pal(256), ...)
  }
}
